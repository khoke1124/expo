...
export async function initAsync(
  ...
  await spawnAsync(
    'pnpm',
    ['create', 'expo', projectName, '--yes', '--no-install', '--template', localTSTemplatePathName],
    {
      cwd: workingDir,
      stdio: 'inherit',
    }
  ...
  if (isTV) {
    prebuildArgs.push(...['--platform', 'tvos']);
  }
  ...
  // We are done with template tarball, but copy it to the test project dependencies folder
  // in case we want to run prebuild again for testing
  const dependenciesPath = path.join(projectRoot, 'dependencies');
  const destinationTemplatePath = path.join(dependenciesPath, path.basename(localTemplatePathName));
  await fs.copyFile(localTemplatePathName, destinationTemplatePath);
